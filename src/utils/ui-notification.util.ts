...
class HTMLCompanyDialogElement extends HTMLCustomDialogElement<any> {
  ...
  setData(data: any) {
    ...
    const showMember = (member: {
      ...
      memberName.innerHTML = `${char.name}, Level ${char.level}`;
      xpPanel.innerHTML = '';
      ...
      xpGaugeRow.innerHTML = `<td title="xp"><strong>XP:</strong></td>
        ...
        class="${
          char.data.core.xp > xpGrowth.xpToUp(char.level) ? 'gold-text' : ''
        }"
        >${char.data.core.xp}</span>/${xpGrowth.xpToUp(
        char.level
      )} </span><progress  class='XP' value='${Math.min(
        (char.data.core.xp / xpGrowth.xpToUp(char.level)) * 100,
        100
      )}' max='100'/></td>`;
      ...
      if (xpGrowth.xpToUp(char.level) <= char.data.core.xp) {
        ...
        levelUpButton.addEventListener('click', (event) => {
          ...
          const bonus = Math.max(
            Math.ceil(
              (((char.level + 1) / 4 + karmaInfluence) *
                (1 + karmaInfluence * Math.random())) /
                10
            ),
            1
          );
          char.data.core.skillPoints +=
            char.data.core.growthPlan.skillPointsOnUp + bonus;
          showMember(member);
        });
      }
        ...
        gaugeRow.innerHTML = `<td title="${
          GAUGE_INFOS[key as GAUGE_KEYS]?.description
            ...
            <td class='gauge-value' ><span>${GaugeCalc.getCurrentValueString(
              char,
              gauge
            )} </span><progress  class='${key}' value='${GaugeCalc.getPercentualValue(
          char,
          gauge
        )}' max='100'/></td>`;
      ...
      skillPoints.innerHTML = `<strong>Skill Points:</strong> ${char.data.core.skillPoints}`;
      ...
      autoBattleInput.checked = char.data.configuration.autoBattle;
      autoBattleInput.addEventListener('change', () => {
        char.data.configuration.autoBattle = autoBattleInput.checked;
      });
      ...
      Object.keys(STAT_KEY).forEach((key: string) => {
        ...
        statRow.innerHTML += `<td title="${
          STAT_INFOS[key as STAT_KEY]?.description
            ...
            (<span class='${
              StatCalc.getInfluenceValue(char, stat) < 0
                ? 'negativo'
                : 'positivo'
            }' >${Math.abs(
          StatCalc.getInfluenceValue(char, stat)
        )}</span>)</td>`;
        statsHolder.appendChild(statRow);
        if (char.data.core.skillPoints > upCost) {
